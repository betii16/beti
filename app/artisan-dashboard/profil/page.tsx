...
export default function ArtisanProfileConfig() {
  ...
  // Géocode une adresse texte en lat/lng — RETOURNE les valeurs
  ...
  const handleSave = async () => {
    ...
    // 3. Sauvegarder artisan — UPSERT pour créer si n'existe pas
    ...
    // Ajouter coordonnées seulement si on les a
    if (saveLat && saveLng) {
    ...
    setSaved(true)
    if (!saveLat || !saveLng) {
      setSaveError('Profil sauvegardé, mais l\'adresse n\'a pas pu être géolocalisée. Vous n\'apparaîtrez pas sur la carte.')
    }
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', background: '#0b0b12', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ fontSize: 14, color: '#4a4a65' }}>Chargement...</div>
      </div>
    )
  }
  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh', background: '#0b0b12', paddingTop: 64, fontFamily: 'Nexa, sans-serif' }}>
        <div style={{ maxWidth: 720, margin: '0 auto', padding: '32px 24px 80px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 }}>
            <div>
              ...
              <h1 style={{ fontSize: 28, fontWeight: 800, color: '#e0dfe5' }}>Mon profil artisan</h1>
            </div>
          ...
          {/* Tabs */}
          <div style={{ display: 'flex', gap: 4, marginBottom: 28, background: '#13131e', borderRadius: 12, padding: 4 }}>
            {([
              ...
              <button key={tab.id} onClick={() => setActiveSection(tab.id)}
                style={{
                  ...
                  color: activeSection === tab.id ? '#6366f1' : '#4a4a65',
              ...
              >
                ...
                {'badge' in tab && (tab as any).badge > 0 && (
                  <span style={{ padding: '1px 7px', borderRadius: 10, background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', color: '#0b0b12', fontSize: 10, fontWeight: 800 }}>{(tab as any).badge}</span>
                )}
              </button>
            ))}
          </div>
          ...
          {activeSection === 'services' && (
            <div style={{ animation: 'fadeIn 0.3s ease' }}>
              ...
              {services.map((service, i) => {
                ...
                return (
                  <div key={i} style={{ background: '#13131e', border: `0.5px solid ${cat?.color || '#1c1c30'}33`, borderRadius: 16, padding: '24px', marginBottom: 16 }}>

                    {/* Header service */}
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        <div style={{ width: 40, height: 40, borderRadius: 10, background: (cat?.color || '#6366f1') + '22', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>{cat?.icon || '🔧'}</div>
                        <div>
                          <div style={{ fontSize: 16, fontWeight: 800, color: '#e0dfe5' }}>{cat?.label || service.category}</div>
                          <div style={{ fontSize: 11, color: '#4a4a65', fontWeight: 300 }}>Service {i + 1}{i === 0 ? ' · Principal' : ''}</div>
                        </div>
                      </div>
                    ...
                    <div style={{ marginBottom: 20 }}>
                      <label style={{ fontSize: 11, color: '#8585a0', fontWeight: 800, letterSpacing: '.06em', display: 'block', marginBottom: 8 }}>
                      ...
                      <textarea
                        ...
                        maxLength={300}
                        style={{
                          width: '100%', padding: '12px 14px', background: '#0b0b12',
                          border: '0.5px solid #1c1c30', borderRadius: 10, color: '#e0dfe5',
                          fontSize: 13, outline: 'none', fontFamily: 'Nexa, sans-serif',
                          fontWeight: 300, resize: 'none', lineHeight: 1.6,
                        }}
                      />
                      <div style={{ fontSize: 10, color: '#4a4a65', textAlign: 'right', marginTop: 4 }}>{service.description.length}/300</div>
                    </div>

                    {/* Type de tarification */}
                    <div style={{ marginBottom: 16 }}>
                      <label style={{ fontSize: 11, color: '#8585a0', fontWeight: 800, letterSpacing: '.06em', display: 'block', marginBottom: 10 }}>
                        TARIFICATION
                      </label>
                        ...
                        {PRICE_TYPES.map(pt => (
                          <button key={pt.id} onClick={() => updateService(i, { price_type: pt.id as any })}
                            style={{
                              ...
                              border: `0.5px solid ${service.price_type === pt.id ? '#6366f1' : '#1c1c30'}`,
                              background: service.price_type === pt.id ? '#6366f10d' : '#0b0b12',
                              color: service.price_type === pt.id ? '#6366f1' : '#4a4a65',
                    ...
                    {(service.price_type === 'hour' || service.price_type === 'service') && (
                      <div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                          <label style={{ fontSize: 11, color: '#8585a0', fontWeight: 800, letterSpacing: '.06em' }}>
                            {service.price_type === 'hour' ? 'TARIF HORAIRE' : 'TARIF PAR PRESTATION'}
                          </label>
                          <div style={{ fontSize: 22, fontWeight: 800, color: '#6366f1' }}>
                            {service.price.toLocaleString('fr-DZ')} <span style={{ fontSize: 12, color: '#4a4a65', fontWeight: 300 }}>{getPriceType(service.price_type)?.suffix}</span>
                          </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                          <span style={{ fontSize: 10, color: '#4a4a65' }}>500 DA</span>
                          <span style={{ fontSize: 10, color: '#4a4a65' }}>30 000 DA</span>
                        </div>
                      </div>
                    )}

                    {service.price_type === 'quote' && (
                      <div style={{ padding: '10px 14px', borderRadius: 8, background: '#0b0b12', border: '0.5px solid #1c1c30' }}>
                        <span style={{ fontSize: 12, color: '#8585a0', fontWeight: 300 }}>📝 Les clients vous contacteront pour obtenir un devis personnalisé</span>
                      </div>
                    )}
                    {service.price_type === 'negotiable' && (
                      <div style={{ padding: '10px 14px', borderRadius: 8, background: '#0b0b12', border: '0.5px solid #1c1c30' }}>
                        <span style={{ fontSize: 12, color: '#8585a0', fontWeight: 300 }}>🤝 Le prix sera discuté avec le client selon la prestation</span>
                      </div>
                    )}
                  </div>
                )
              ...
              {!addingService ? (
                ...
                <div style={{ background: '#13131e', border: '0.5px solid #6366f133', borderRadius: 16, padding: '24px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                    <div style={{ fontSize: 14, fontWeight: 800, color: '#e0dfe5' }}>Choisir un métier</div>
                    <button onClick={() => setAddingService(false)} style={{ background: 'transparent', border: 'none', color: '#4a4a65', cursor: 'pointer', fontSize: 16 }}>✕</button>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                    {CATEGORIES.filter(c => !services.find(s => s.category === c.id)).map(cat => (
                      <button key={cat.id} onClick={() => addService(cat.id)}
                        style={{
                          padding: '14px 10px', borderRadius: 12, border: '0.5px solid #1c1c30',
                          background: '#0b0b12', cursor: 'pointer', display: 'flex',
                        ...
                        onMouseLeave={e => { e.currentTarget.style.borderColor = '#1c1c30'; e.currentTarget.style.background = '#0b0b12' }}
                      >
                        <span style={{ fontSize: 22 }}>{cat.icon}</span>
                        <span style={{ fontSize: 11, color: '#8585a0', fontWeight: 300 }}>{cat.label}</span>
                      </button>
                    ))}
                  </div>
                  {services.length === 0 && (
                    <p style={{ fontSize: 11, color: '#4a4a65', fontWeight: 300, marginTop: 12, textAlign: 'center' }}>
          ...
          {activeSection === 'info' && (
            <div style={{ animation: 'fadeIn 0.3s ease' }}>
              <div style={{ background: '#13131e', border: '0.5px solid #1c1c30', borderRadius: 16, padding: '24px' }}>
                ...
                {/* Nom */}
                <div style={{ marginBottom: 20 }}>
                  <label style={{ fontSize: 11, color: '#8585a0', fontWeight: 800, letterSpacing: '.06em', display: 'block', marginBottom: 8 }}>NOM COMPLET</label>
                  <input type="text" value={fullName} onChange={e => setFullName(e.target.value)}
                    style={{ width: '100%', padding: '13px 16px', background: '#0b0b12', border: '0.5px solid #1c1c30', borderRadius: 10, color: '#e0dfe5', fontSize: 14, outline: 'none', fontFamily: 'Nexa, sans-serif', fontWeight: 300 }}/>
                </div>
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <div style={{ padding: '13px 14px', background: '#0b0b12', border: '0.5px solid #1c1c30', borderRadius: 10, color: '#8585a0', fontSize: 14, fontWeight: 300, flexShrink: 0 }}>
                      +213
                    </div>
                    <input type="tel" value={phone} placeholder="0555 12 34 56"
                      onChange={e => { setPhone(e.target.value); setPhoneError('') }}
                      onBlur={() => checkPhone(phone)}
                      style={{ flex: 1, padding: '13px 16px', background: '#0b0b12', border: `0.5px solid ${phoneError ? '#ef4444' : '#1c1c30'}`, borderRadius: 10, color: '#e0dfe5', fontSize: 14, outline: 'none', fontFamily: 'Nexa, sans-serif', fontWeight: 300 }}/>
                  </div>
                  {phoneError && <p style={{ fontSize: 12, color: '#ef4444', marginTop: 6, fontWeight: 300 }}>{phoneError}</p>}
                  <p style={{ fontSize: 11, color: '#4a4a65', marginTop: 6, fontWeight: 300 }}>
                ...
                {/* Bio */}
                <div style={{ marginBottom: 20 }}>
                  <label style={{ fontSize: 11, color: '#8585a0', fontWeight: 800, letterSpacing: '.06em', display: 'block', marginBottom: 8 }}>BIO / PRÉSENTATION</label>
                  <textarea value={bio} onChange={e => setBio(e.target.value)}
                    placeholder="Présentez-vous en quelques mots... (parcours, expérience, ce qui vous distingue)"
                    rows={4} maxLength={500}
                    style={{ width: '100%', padding: '12px 14px', background: '#0b0b12', border: '0.5px solid #1c1c30', borderRadius: 10, color: '#e0dfe5', fontSize: 13, outline: 'none', fontFamily: 'Nexa, sans-serif', fontWeight: 300, resize: 'none', lineHeight: 1.6 }}/>
                  <div style={{ fontSize: 10, color: '#4a4a65', textAlign: 'right', marginTop: 4 }}>{bio.length}/500</div>
                </div>

                {/* Expérience */}
                <div style={{ marginBottom: 20 }}>
                  <label style={{ fontSize: 11, color: '#8585a0', fontWeight: 800, letterSpacing: '.06em', display: 'block', marginBottom: 10 }}>ANNÉES D'EXPÉRIENCE</label>
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {[1, 2, 3, 5, 7, 10, 15, 20].map(y => (
                      <button key={y} onClick={() => setExperience(y)}
                        style={{ padding: '8px 16px', borderRadius: 10, border: `0.5px solid ${experience === y ? '#6366f1' : '#1c1c30'}`, background: experience === y ? '#6366f10d' : '#0b0b12', color: experience === y ? '#6366f1' : '#4a4a65', fontSize: 13, fontWeight: experience === y ? 800 : 300, cursor: 'pointer', fontFamily: 'Nexa, sans-serif' }}>
                        {y} an{y > 1 ? 's' : ''}
                      </button>
                    ))}
                  </div>
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  <p style={{ fontSize: 11, color: '#4a4a65', fontWeight: 300, marginBottom: 10 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <input
                      ...
                      style={{ flex: 1, padding: '13px 16px', background: '#0b0b12', border: `0.5px solid ${addressError ? '#ef4444' : '#1c1c30'}`, borderRadius: 10, color: '#e0dfe5', fontSize: 14, outline: 'none', fontFamily: 'Nexa, sans-serif', fontWeight: 300 }}
                    />
                    <button onClick={detectPosition} disabled={detectingAddr}
                      style={{ padding: '13px 16px', background: '#0b0b12', border: '0.5px solid #1c1c30', borderRadius: 10, color: '#8585a0', fontSize: 12, cursor: 'pointer', fontFamily: 'Nexa, sans-serif', fontWeight: 300, flexShrink: 0, transition: 'all 0.2s' }}
                    >
                  ...
                  {artisanLat && artisanLng && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 8 }}>
                      ...
                      <span style={{ fontSize: 11, color: '#10b981', fontWeight: 300 }}>Position enregistrée</span>
                    </div>
                  )}
                </div>
                ...
                <div style={{ marginBottom: 20 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                    <label style={{ fontSize: 11, color: '#8585a0', fontWeight: 800, letterSpacing: '.06em' }}>RAYON D'INTERVENTION</label>
                    <span style={{ fontSize: 18, fontWeight: 800, color: '#6366f1' }}>{radiusKm} km</span>
                  </div>
                  ...
                  />
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                    <span style={{ fontSize: 10, color: '#4a4a65' }}>5 km</span>
                    <span style={{ fontSize: 10, color: '#4a4a65' }}>100 km</span>
                  </div>
                </div>

                {/* Photo de profil */}
                <div>
                  <label style={{ fontSize: 11, color: '#8585a0', fontWeight: 800, letterSpacing: '.06em', display: 'block', marginBottom: 12 }}>PHOTO DE PROFIL</label>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    ...
                    <div>
                      <p style={{ fontSize: 13, color: '#8585a0', fontWeight: 300, marginBottom: 4 }}>Une photo aide les clients à vous faire confiance</p>
                      <p style={{ fontSize: 11, color: '#4a4a65', fontWeight: 300 }}>JPG ou PNG · Max 5MB</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {activeSection === 'tags' && (
            <div style={{ animation: 'fadeIn 0.3s ease' }}>
              <div style={{ background: '#13131e', border: '0.5px solid #1c1c30', borderRadius: 16, padding: '24px' }}>
                ...
                <div style={{ marginBottom: 16 }}>
                  <div style={{ fontSize: 14, fontWeight: 800, color: '#e0dfe5', marginBottom: 6 }}>Mots-clés & spécialités</div>
                  <p style={{ fontSize: 12, color: '#4a4a65', fontWeight: 300, lineHeight: 1.6 }}>
                    Ajoutez des mots-clés pour décrire vos spécialités. Les clients qui cherchent ces termes vous trouveront plus facilement.
                    {services.length === 0 && <><br/><span style={{ color: '#6366f1' }}>Si vous n'avez sélectionné aucun métier standard, vos mots-clés sont essentiels !</span></>}
                  </p>
                </div>

                {/* Zone de saisie tags */}
                <div style={{ minHeight: 52, padding: '8px 12px', background: '#0b0b12', border: '0.5px solid #1c1c30', borderRadius: 12, display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center', marginBottom: 12 }}>
                  {tags.map(tag => (
                  ...
                  <input
                    ...
                    style={{ flex: 1, minWidth: 120, background: 'transparent', border: 'none', outline: 'none', color: '#e0dfe5', fontSize: 13, fontFamily: 'Nexa, sans-serif', fontWeight: 300, padding: '4px' }}
                  />
                  {tagInput.trim() && (
                    <button onClick={() => addTag(tagInput)}
                      style={{ padding: '4px 14px', borderRadius: 20, background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', border: 'none', color: '#0b0b12', fontSize: 11, fontWeight: 800, cursor: 'pointer', fontFamily: 'Nexa, sans-serif', flexShrink: 0 }}>
                      + Ajouter
                    </button>
                  )}
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                  <span style={{ fontSize: 11, color: '#4a4a65', fontWeight: 300 }}>{tags.length}/15 mots-clés</span>
                  <div style={{ display: 'flex', gap: 4 }}>
                    {Array.from({ length: 15 }).map((_, i) => (
                      <div key={i} style={{ width: 6, height: 6, borderRadius: 2, background: i < tags.length ? '#6366f1' : '#1c1c30' }}/>
                    ))}
                  </div>
                </div>
                ...
                {tags.length < 15 && (
                  <div>
                    <div style={{ fontSize: 10, color: '#4a4a65', fontWeight: 800, letterSpacing: '.08em', marginBottom: 8 }}>SUGGESTIONS</div>
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                      {POPULAR_TAGS.filter(t => !tags.includes(t)).slice(0, 10).map(t => (
                        <button key={t} onClick={() => addTag(t)}
                          style={{ padding: '5px 14px', borderRadius: 20, background: 'transparent', border: '0.5px solid #1c1c30', color: '#4a4a65', fontSize: 12, cursor: 'pointer', fontFamily: 'Nexa, sans-serif', fontWeight: 300, transition: 'all 0.15s' }}
                          onMouseEnter={e => { e.currentTarget.style.borderColor = '#6366f1'; e.currentTarget.style.color = '#6366f1' }}
                          onMouseLeave={e => { e.currentTarget.style.borderColor = '#1c1c30'; e.currentTarget.style.color = '#4a4a65' }}
                        >+ {t}</button>
                      ))}
                    </div>
                  </div>
          ...
          <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'linear-gradient(transparent, #0b0b12 30%)', padding: '40px 24px 24px', zIndex: 50 }}>
