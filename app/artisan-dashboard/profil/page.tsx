...
const PRICE_TYPES = [
  { id: 'hour',       label: 'À l\'heure',      suffix: 'DA/h',       Icon: Clock },
  { id: 'service',    label: 'Par prestation',   suffix: 'DA/prestation', Icon: ClipboardList },
  { id: 'quote',      label: 'Sur devis',        suffix: '',           Icon: FileText },
  { id: 'negotiable', label: 'À négocier',       suffix: '',           Icon: Handshake },
]
...
export default function ArtisanProfileConfig() {
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ fontSize: 14, color: 'var(--tx3)' }}>Chargement...</div>
      </div>
    )
  }
  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh', background: 'var(--bg)', paddingTop: 64, fontFamily: 'Nexa, sans-serif' }}>
        <div style={{ maxWidth: 720, margin: '0 auto', padding: '32px 24px 80px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 }}>
            <div>
              ...
              <h1 style={{ fontSize: 28, fontWeight: 800, color: 'var(--tx)' }}>Mon profil artisan</h1>
            </div>
          ...
          <div style={{ display: 'flex', gap: 4, marginBottom: 28, background: 'var(--bg2)', borderRadius: 12, padding: 4 }}>
            {([
              { id: 'services', label: 'Mes métiers', Icon: Wrench, badge: services.length },
              { id: 'info',     label: 'Informations', Icon: User },
              { id: 'tags',     label: 'Mots-clés', Icon: Tag, badge: tags.length },
              { id: 'portfolio', label: 'Réalisations', Icon: Hammer },
            ] as const).map(tab => (
                  ...
                  <div key={i} style={{ background: 'var(--bg2)', border: `0.5px solid ${cat?.color || 'var(--border)'}33`, borderRadius: 16, padding: '24px', marginBottom: 16 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        ...
                        <div>
                          <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--tx)' }}>{cat?.label || service.category}</div>
                          <div style={{ fontSize: 11, color: 'var(--tx3)', fontWeight: 300 }}>Service {i + 1}{i === 0 ? ' · Principal' : ''}</div>
                        </div>
                      </div>
                        ...
                        {PRICE_TYPES.map(pt => (
                          <button key={pt.id} onClick={() => updateService(i, { price_type: pt.id as any })}
                            style={{
                              ...
                              border: `0.5px solid ${service.price_type === pt.id ? '#6366f1' : 'var(--border)'}`,
                              background: service.price_type === pt.id ? '#6366f10d' : 'var(--bg)',
                              color: service.price_type === pt.id ? '#6366f1' : 'var(--tx3)',
                              fontSize: 12, fontWeight: service.price_type === pt.id ? 800 : 300,
                              cursor: 'pointer', fontFamily: 'Nexa, sans-serif',
                              display: 'flex', alignItems: 'center', gap: 8, transition: 'all 0.15s',
                            }}
                          >
                            <pt.Icon size={15}/> {pt.label}
                          </button>
                        ))}
                      </div>
                    </div>
                      ...
                      <div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                          ...
                          <div style={{ fontSize: 22, fontWeight: 800, color: '#6366f1' }}>
                            {service.price.toLocaleString('fr-DZ')} <span style={{ fontSize: 12, color: 'var(--tx3)', fontWeight: 300 }}>{getPriceType(service.price_type)?.suffix}</span>
                          </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                          <span style={{ fontSize: 10, color: 'var(--tx3)' }}>500 DA</span>
                          <span style={{ fontSize: 10, color: 'var(--tx3)' }}>30 000 DA</span>
                        </div>
                      </div>
                    )}
                    ...
                    {service.price_type === 'quote' && (
                      <div style={{ padding: '10px 14px', borderRadius: 8, background: 'var(--bg)', border: '0.5px solid var(--border)' }}>
                        <span style={{ fontSize: 12, color: 'var(--tx2)', fontWeight: 300 }}><FileText size={12} style={{ verticalAlign: 'middle', marginRight: 4 }}/>Les clients vous contacteront pour obtenir un devis personnalisé</span>
                      </div>
                    )}
                    {service.price_type === 'negotiable' && (
                      <div style={{ padding: '10px 14px', borderRadius: 8, background: 'var(--bg)', border: '0.5px solid var(--border)' }}>
                        <span style={{ fontSize: 12, color: 'var(--tx2)', fontWeight: 300 }}><Handshake size={12} style={{ verticalAlign: 'middle', marginRight: 4 }}/>Le prix sera discuté avec le client selon la prestation</span>
                      </div>
                    )}
                  </div>
                )
              ...
              {!addingService ? (
                ...
                <div style={{ background: 'var(--bg2)', border: '0.5px solid #6366f133', borderRadius: 16, padding: '24px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                    <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--tx)' }}>Choisir un métier</div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                    {CATEGORIES.filter(c => !services.find(s => s.category === c.id)).map(cat => (
                      ...
                      >
                        ...
                        <span style={{ fontSize: 11, color: 'var(--tx2)', fontWeight: 300 }}>{cat.label}</span>
                      </button>
                    ))}
                  </div>
                  {services.length === 0 && (
          ...
          {activeSection === 'info' && (
            <div style={{ animation: 'fadeIn 0.3s ease' }}>
              <div style={{ background: 'var(--bg2)', border: '0.5px solid var(--border)', borderRadius: 16, padding: '24px' }}>
                ...
                {/* Nom */}
                <div style={{ marginBottom: 20 }}>
                  <label style={{ fontSize: 11, color: 'var(--tx2)', fontWeight: 800, letterSpacing: '.06em', display: 'block', marginBottom: 8 }}>NOM COMPLET</label>
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  <p style={{ fontSize: 11, color: 'var(--tx3)', marginTop: 6, fontWeight: 300 }}>
                    <Lock size={12} style={{ verticalAlign: 'middle', marginRight: 4 }}/>Un seul compte par numéro — les clients pourront vous appeler directement
                  </p>
                </div>

                {/* Bio */}
                <div style={{ marginBottom: 20 }}>
                  <label style={{ fontSize: 11, color: 'var(--tx2)', fontWeight: 800, letterSpacing: '.06em', display: 'block', marginBottom: 8 }}>BIO / PRÉSENTATION</label>
                ...
                {/* Expérience */}
                <div style={{ marginBottom: 20 }}>
                  <label style={{ fontSize: 11, color: 'var(--tx2)', fontWeight: 800, letterSpacing: '.06em', display: 'block', marginBottom: 10 }}>ANNÉES D'EXPÉRIENCE</label>
                ...
                <div style={{ marginBottom: 20 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                    <label style={{ fontSize: 11, color: 'var(--tx2)', fontWeight: 800, letterSpacing: '.06em' }}>RAYON D'INTERVENTION</label>
                  ...
                  />
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                    <span style={{ fontSize: 10, color: 'var(--tx3)' }}>5 km</span>
                    <span style={{ fontSize: 10, color: 'var(--tx3)' }}>100 km</span>
                  </div>
                </div>

                {/* Photo de profil */}
                <div>
                  <label style={{ fontSize: 11, color: 'var(--tx2)', fontWeight: 800, letterSpacing: '.06em', display: 'block', marginBottom: 12 }}>PHOTO DE PROFIL</label>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    ...
                    <div>
                      <p style={{ fontSize: 13, color: 'var(--tx2)', fontWeight: 300, marginBottom: 4 }}>Une photo aide les clients à vous faire confiance</p>
                      <p style={{ fontSize: 11, color: 'var(--tx3)', fontWeight: 300 }}>JPG ou PNG · Max 5MB</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {activeSection === 'tags' && (
            <div style={{ animation: 'fadeIn 0.3s ease' }}>
              <div style={{ background: 'var(--bg2)', border: '0.5px solid var(--border)', borderRadius: 16, padding: '24px' }}>
                ...
                <div style={{ marginBottom: 16 }}>
                  <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--tx)', marginBottom: 6 }}>Mots-clés & spécialités</div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                  <span style={{ fontSize: 11, color: 'var(--tx3)', fontWeight: 300 }}>{tags.length}/15 mots-clés</span>
                ...
                {tags.length < 15 && (
                  <div>
                    <div style={{ fontSize: 10, color: 'var(--tx3)', fontWeight: 800, letterSpacing: '.08em', marginBottom: 8 }}>SUGGESTIONS</div>
          ...
          <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'linear-gradient(transparent, var(--bg) 30%)', padding: '40px 24px 24px', zIndex: 50 }}>
